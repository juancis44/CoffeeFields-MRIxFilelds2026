...
def _clean(val):
    """Strip whitespace/CR from an env value.

    A .env saved with Windows CRLF endings and then `source`d in bash leaves a
    trailing '\r' inside every value (e.g. DEVICE='cuda:0\r'), which breaks
    torch.device() and creates bogus directory names. Values set directly in the
    shell take precedence over .env, so we sanitize on read.
    """
...
def get_data_dir() -> str:
    ...
    load_env()
    val = _clean(os.environ.get("DATA_DIR"))
    if not val:
        raise RuntimeError(
...
def get_preprocessed_dir() -> str | None:
    ...
    load_env()
    val = _clean(os.environ.get("PREPROCESSED_DIR"))
...
def get_output_dir() -> str:
    ...
    load_env()
    val = _clean(os.environ.get("OUTPUT_DIR"))
    if not val:
        raise RuntimeError(
...
def get_inference_dir() -> str:
    ...
    load_env()
    val = _clean(os.environ.get("INFERENCE_DIR"))
    if not val:
        raise RuntimeError(
...
def get_predictions_seg_dir() -> str:
    ...
    load_env()
    val = _clean(os.environ.get("PREDICTIONS_SEG_DIR"))
    if not val:
        raise RuntimeError(
...
def get_submission_dir() -> str:
    ...
    load_env()
    val = _clean(os.environ.get("SUBMISSION_DIR"))
    if not val:
        raise RuntimeError(
...
def get_synthseg_dir() -> str:
    ...
    load_env()
    val = _clean(os.environ.get("SYNTHSEG_DIR"))
    if not val:
        raise RuntimeError(
...
def get_device() -> str:
    ...
    load_env()
    return _clean(os.environ.get("DEVICE")) or "cuda:0"
